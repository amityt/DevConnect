...
//@desc    Get profile by id,user & posts
//@access  Private
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      profile.experience.unshift(newExp); //Pushes data in beginning rather than at the end
      await profile.save();
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      const profile = await Profile.findOne({ user: req.user.id });
      profile.education.unshift(newEdu); //Pushes data in beginning rather than at the end
      await profile.save();
